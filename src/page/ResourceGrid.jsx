...
export default function ResourcesGrid() {
  ...
  return (
    <section className="min-h-screen bg-[#0f172a] py-12 px-4 sm:px-8 lg:px-12 text-slate-300">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-10 flex flex-col gap-4">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
            ...
            {/* Selects */}
            {[
              { label: "Difficulté", icon: <Filter size={14} /> },
              { label: "Type", icon: <BookOpen size={14} /> },
              { label: "Trier par", icon: <ArrowUpDown size={14} /> },
              { label: "Sélectionner des balises", icon: <Filter size={14} /> }
            ].map((filter, i) => (
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:gap-8">
          {resources.map((item, index) => (
            <motion.div
              ...
              className="group flex flex-col justify-between rounded-2xl border border-slate-800 bg-[#1e293b]/40 p-6 transition-all hover:border-emerald-500/30 hover:bg-[#1e293b] hover:shadow-2xl hover:shadow-emerald-900/10"
            >
              <div>
                <h3 className="text-lg font-bold text-slate-100 leading-snug group-hover:text-emerald-400 transition-colors line-clamp-2">
                  {item.title}
                </h3>
              ...
              <div className="mt-8 flex items-center justify-between">
                <button className="rounded-lg bg-emerald-600/10 px-6 py-2 text-sm font-bold text-emerald-500 border border-emerald-500/20 transition-all hover:bg-emerald-500 hover:text-white hover:shadow-lg hover:shadow-emerald-900/20">
                  Explorer
                </button>
                
                {/* Micro-décoration */}
                <div className="h-0.5 w-0 bg-emerald-500 transition-all duration-500 group-hover:w-16"></div>
              </div>
            </motion.div>
          ))}
        </div>
