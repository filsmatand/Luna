...
import { Smartphone, Wallet, CreditCard, CheckCircle } from 'lucide-react'; // Import Lucide icons
...
export default function PaymentGate() {
  ...
  const paymentMethods = [
    ...
    { id: 'mpesa', name: 'M-PESA', icon: Wallet },
    { id: 'orange_money', name: 'Orange Money', icon: Wallet },
    { id: 'paypal', name: 'PayPal', icon: Wallet },
    { id: 'credit_card', name: 'Carte Bancaire', icon: CreditCard },
  ];
                ...
                {prices.map((price) => (
                  ...
                  >
                    ...
                    {selectedPrice && selectedPrice.id === price.id && (
                      <CheckCircle className="absolute top-3 right-3 text-blue-400 w-6 h-6" />
                    )}
                  </div>
                ))}
              </div>
                    ...
                    return (
                      ...
                      >
                        <div className="text-4xl mb-3 flex justify-center items-center">
                          <IconComponent className="w-10 h-10 text-gray-200 group-hover:text-white" />
                        </div>
                        ...
                        {selectedPaymentMethod && selectedPaymentMethod.id === method.id && (
                          <CheckCircle className="absolute top-3 right-3 text-purple-400 w-6 h-6" />
                        )}
                      </div>
                    );
                  })}
